...
export class FolderTreeComponent implements OnInit {
  ...
  async  viewImage(data) {
    const extension = this.getFileExtension(data);
    if (extension === "pdf") {
      ...
      // Otherwise, use the path URL directly
      ...
      if (fileHash) {
        previewData.fileHash = fileHash;
        previewData.file_hash = fileHash;
        previewData.file = {
          fileHash: fileHash,
          file_hash: fileHash,
          mime_type: "application/pdf",
        ...
        console.log('Navigating to PdfPreviewComponent with fileHash for API call');
      } else if (filePath) {
        // Fallback to path URL if no fileHash
        previewData.file_path = filePath;
        previewData.path = filePath;
        previewData.pdfUrl = filePath;
        previewData.file = {
          path: filePath,
          file_path: filePath,
          mime_type: "application/pdf",
        ...
        console.log('Navigating to PdfPreviewComponent with file path URL');
      } else {
        this._snackBar.open("File path or fileHash not available", "Close", {
          duration: 3000,
          horizontalPosition: "right",
          verticalPosition: "top",
          panelClass: ["error-snackbar"],
      ...
      this._router.navigate(['/folder-tree/folder-pdf-view'], {
