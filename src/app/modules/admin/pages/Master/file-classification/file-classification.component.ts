...
import { AddUpdateFileClassComponent } from './add-update-file-class/add-update-file-class.component';

@Component({
  selector: 'app-file-classification',
  standalone: true,
  imports: [
    NgIf,
    RouterLink,
  ...
  styleUrl: './file-classification.component.scss'
})
export class FileClassificationComponent implements OnInit {
 ...
 "action"
  ...
  addClassification(data) {
    const dialogRef = this.dialog.open(AddUpdateFileClassComponent, {
      data: data,
      width: "400px",
    });
  ...
  editFilesClassification(row: any): void {
    this.addClassification(row);
  }
  ...
  deleteFilesClassification(data) {
        this._masterService.deleteFileClassification(data.fileClassificationId).subscribe({
        next: (response: any) => {
          this._snackBar.open("File Classification Deleted successfully", "Close", {
            duration: 3000,
